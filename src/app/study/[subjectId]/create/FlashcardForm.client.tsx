...
export default function FlashcardForm({ subject, subjectId }: FlashcardFormProps) {
  ...
  const [isLoading, setIsLoading] = useState(subject.name === 'Carregando...');
  const [cards, setCards] = useState<{ question: string; answer: string }[]>([]);

  // Verificar autenticação e carregar dados se necessário
  useEffect(() => {
    const checkAuthAndLoadData = async () => {
      ...
      // Se o subject veio com o nome padrão "Carregando...", buscamos os dados corretos
      if (subject.name === 'Carregando...') {
        setIsLoading(true);
        try {
  ...
  }, [subject, subjectId, toast, router]);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-3xl font-bold">{subjectData?.name || 'Criar Flashcards'}</h1>
      </div>
      
      {isLoading ? (
        <Card className="p-6 flex items-center justify-center">
          <p className="text-lg">Carregando dados da matéria...</p>
        </Card>
      ) : (
        <>
            ...
            <div>
              ...
              <div className="space-y-4">
                {cards.map((card, index) => (
                  <Card key={index} className="p-4 border-l-4" style={{ borderLeftColor: subjectData?.color || '#4F46E5' }}>
